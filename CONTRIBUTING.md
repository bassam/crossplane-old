...
The Conductor project is (about to be) under [Apache 2.0 license](LICENSE). We accept contributions via
...
#### Kubebuilder inside Conductor Project

##### kubebilder
You can also run `kubebuilder` inside the Conductor project to create new types and controllers, which you can do either via kubebuilder command:
```
kubebuilder init --domain aws[gcp].conductor.io
kubebuilder create api --group <group-name> --version <v1alpha1> --kind <Kind-name> 
```

**Note** kubebuilder will create files that do not follow this project organization. You will need to move the created artifacts and update their content.  

##### make
You can also generate new resource using `Makefile`, via:
```
make kubebuilder-create [API_DOMAIN=<domain-name>, default: conductor.io] [API_PROVIDER=<aws[gcp]>, default: aws] API_GROUP=<group-anme> API_KIND=<Kind-Name> [API_VERSION=<version>, default: v1alpha1]  
```

**Note** make wil create files that "almost" follow this project organization: `apis` files are expected to be place in correct packages, whereas, `controllers` are placed inside `./pkg/` (just like in `kubebuilder` options).
You will need to move the created artifacts and update their content.

To verify that new artifacts run: 
```bash
make build test
```
To locally test new types/controllers please refer to [Local Build](/build/local/README.md) instructions.
